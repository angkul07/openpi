...
class DataConfigFactory(abc.ABC):
    ...
    def _load_norm_stats(self, assets_dir: epath.Path, asset_id: str | None) -> dict[str, _transforms.NormStats] | None:
        ...
        try:
            ...
            local_dir = _download.maybe_download(data_assets_dir)
            norm_stats = _normalize.load(local_dir)
            # Verify these stats were computed for THIS distribution. Mixture stats are
            # computed through the training sampler, so a copied directory or a reused
            # asset_id silently normalises against the wrong distribution -- and
            # run_yam.sh skips recomputation whenever the file already exists. Stats
            # written before fingerprinting have none, and warn rather than fail.
            _fingerprint.check(
                local_dir,
                _fingerprint.from_data_config_factory(self),
                config_name=pathlib.Path(str(assets_dir)).name,
                asset_id=asset_id,
            )
...
# Configs defined in THIS file: upstream openpi's own recipes. Experiment arms are
# deliberately not here -- they live in `configs/<client>/...` and register themselves.
# See `openpi.training.registry` for why, and `configs/README.md` for how to add one.
_BUILTIN_CONFIGS_DICT = {config.name: config for config in _CONFIGS}


def _configs_dict() -> dict[str, TrainConfig]:
    """Every available config: the built-ins above, plus the client tree.

    Discovery is LAZY and runs once, on first lookup, rather than at import time --
    client config modules import this module to get `TrainConfig` and friends, so
    discovering at import time would be a cycle.
    """
    discovered = registry.registered()
    if collisions := discovered.keys() & _BUILTIN_CONFIGS_DICT.keys():
        owners = ", ".join(f"{name} (from {registry.owner(name)})" for name in sorted(collisions))
        raise ValueError(
            f"client configs shadow built-in openpi configs: {owners}. "
            "Rename them -- prefix client arms with the client slug, e.g. 'mm_pi05_ea'."
        )
    return _BUILTIN_CONFIGS_DICT | discovered


def config_names() -> list[str]:
    """Sorted names of every available config, built-in and client."""
