...
One directory per client under `configs/` (see [§6](#6-the-config-problem)): FD's own
R&D in `configs/fd/yam/`, mm's in `configs/mm/so101/`. Batch 64, `gemma_2b_lora`,
cosine 3.5e-5 → 3.5e-6, EMA off throughout unless noted.
...
### mm / SO-101 — sim vs retargeted ego (`configs/mm/so101/sim_vs_ego.py`)

The first client outside `fd/`, and the first non-YAM embodiment to land. Three π₀.₅
arms asking whether retargeted ego video buys anything on top of nine minutes of sim,
on a single-arm SO-101 doing *"pick up the cube and place it in the tray"*.
...
they cannot drift into the step-count confound that stopped the π₀.₅ mixture study from
ranking its own arms on training loss.
...
- **Neither pool is trainable as it stands.** The sim dataset is LeRobot **v3.0** and
  ...
  needs retargeting output, multiview synthesis and a v2.1 conversion. Four schema
  contracts the ego conversion has to meet — column order, gripper units, camera keys,
  and letterbox geometry — are enumerated in `configs/mm/datasets.py`.
- **`SO101` is the first spec with `arms=1` and fewer than 6 joints.** `action_dim=6`
  and the `(5, −1)` delta mask both fall out of `RobotSpec` with no special-casing,
  which is the parameterisation from §6 paying off. At 30 Hz its 50-step chunk is
  1.67 s — the same as YAM's, so `action_horizon` is comparable across the two.

The split manifest (`configs/mm/sim_splits.json`) is generated by
`configs/mm/make_sim_splits.py` and checked in, so every arm withholds the same eval
episodes — the only reason the three are rankable against each other.
...
| `configs/mm/` | The first external client — `so101/sim_vs_ego.py`, plus a checked-in split manifest and its generator. Adding it touched no openpi file except to add one `RobotSpec`. |
